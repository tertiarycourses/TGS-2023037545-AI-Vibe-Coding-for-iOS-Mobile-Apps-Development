...
@main struct Activity04QualityGateApp: App {
    ...
    var body: some View {
        NavigationStack {
            VStack(alignment: .leading, spacing: 20) {
                Text("Expose the clean-build gates that generated code must satisfy.").foregroundStyle(.secondary)

                Text("4 / 4").font(.largeTitle.bold()).accessibilityIdentifier("metric")
                ...
                Spacer()
                EvidenceBadge(evidence: ProjectEvidence(statement: "Warnings, CTest, iOS build and evidence green", verifiedLocally: true))
            }
            .padding(20)
            .background(.regularMaterial, in: RoundedRectangle(cornerRadius: 24))
            .padding()
            .navigationTitle("Quality Gate")
            .tint(.indigo)
        }
    }
}
