...
cd "$ROOT_DIR"
if [ ! -d "Activity04QualityGate.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "Activity04QualityGate.xcodeproj" -scheme "Activity04QualityGate" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/Activity04QualityGate-DerivedData" CODE_SIGNING_ALLOWED=NO build
