...
cd "$ROOT_DIR"
if [ ! -d "Activity10ReleaseGate.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "Activity10ReleaseGate.xcodeproj" -scheme "Activity10ReleaseGate" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/Activity10ReleaseGate-DerivedData" CODE_SIGNING_ALLOWED=NO build
