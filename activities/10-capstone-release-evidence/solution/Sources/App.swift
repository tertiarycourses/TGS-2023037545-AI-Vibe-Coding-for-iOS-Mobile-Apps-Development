...
@main struct Activity10ReleaseGateApp: App {
    ...
    var body: some View {
        NavigationStack {
            VStack(alignment: .leading, spacing: 20) {
                Text("Separate local proof from signing, upload and App Review gates.").foregroundStyle(.secondary)

                Text("HOLD").font(.largeTitle.bold()).accessibilityIdentifier("metric")
                ...
                Spacer()
                EvidenceBadge(evidence: ProjectEvidence(statement: "External submission remains a future credentialed gate", verifiedLocally: true))
            }
            .padding(20)
