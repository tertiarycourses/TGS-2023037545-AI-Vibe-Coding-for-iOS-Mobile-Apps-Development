...
cd "$ROOT_DIR"
if [ ! -d "FocusCards.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "FocusCards.xcodeproj" -scheme "FocusCards" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/FocusCards-DerivedData" CODE_SIGNING_ALLOWED=NO build
