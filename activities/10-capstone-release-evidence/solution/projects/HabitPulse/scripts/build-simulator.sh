...
cd "$ROOT_DIR"
if [ ! -d "HabitPulse.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "HabitPulse.xcodeproj" -scheme "HabitPulse" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/HabitPulse-DerivedData" CODE_SIGNING_ALLOWED=NO build
