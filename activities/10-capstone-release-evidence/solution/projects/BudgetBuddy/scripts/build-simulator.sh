...
cd "$ROOT_DIR"
if [ ! -d "BudgetBuddy.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "BudgetBuddy.xcodeproj" -scheme "BudgetBuddy" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/BudgetBuddy-DerivedData" CODE_SIGNING_ALLOWED=NO build
