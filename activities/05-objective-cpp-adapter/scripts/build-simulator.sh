...
cd "$ROOT_DIR"
if [ ! -d "Activity05Bridge.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "Activity05Bridge.xcodeproj" -scheme "Activity05Bridge" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/Activity05Bridge-DerivedData" CODE_SIGNING_ALLOWED=NO build
