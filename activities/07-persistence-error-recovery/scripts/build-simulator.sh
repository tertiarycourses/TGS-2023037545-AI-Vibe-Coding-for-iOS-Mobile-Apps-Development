...
cd "$ROOT_DIR"
if [ ! -d "Activity07Recovery.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "Activity07Recovery.xcodeproj" -scheme "Activity07Recovery" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/Activity07Recovery-DerivedData" CODE_SIGNING_ALLOWED=NO build
