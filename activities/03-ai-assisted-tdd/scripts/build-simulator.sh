...
cd "$ROOT_DIR"
if [ ! -d "Activity03RepairLoop.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "Activity03RepairLoop.xcodeproj" -scheme "Activity03RepairLoop" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/Activity03RepairLoop-DerivedData" CODE_SIGNING_ALLOWED=NO build
