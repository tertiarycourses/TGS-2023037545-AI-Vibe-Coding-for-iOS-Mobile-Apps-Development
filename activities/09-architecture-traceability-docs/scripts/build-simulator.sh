...
cd "$ROOT_DIR"
if [ ! -d "Activity09Architecture.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "Activity09Architecture.xcodeproj" -scheme "Activity09Architecture" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/Activity09Architecture-DerivedData" CODE_SIGNING_ALLOWED=NO build
