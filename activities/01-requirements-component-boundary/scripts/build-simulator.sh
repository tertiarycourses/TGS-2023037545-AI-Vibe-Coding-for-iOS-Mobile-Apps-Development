...
cd "$ROOT_DIR"
if [ ! -d "Activity01ScopeBoard.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "Activity01ScopeBoard.xcodeproj" -scheme "Activity01ScopeBoard" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/Activity01ScopeBoard-DerivedData" CODE_SIGNING_ALLOWED=NO build
