...
cd "$ROOT_DIR"
if [ ! -d "Activity06BudgetUI.xcodeproj" ]; then xcodegen generate --spec project.yml; fi
...
xcodebuild -project "Activity06BudgetUI.xcodeproj" -scheme "Activity06BudgetUI" -destination "platform=iOS Simulator,name=$DEVICE" -derivedDataPath "${TMPDIR:-/tmp}/Activity06BudgetUI-DerivedData" CODE_SIGNING_ALLOWED=NO build
